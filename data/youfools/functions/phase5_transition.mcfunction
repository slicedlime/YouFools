...
execute if score @s YouFools matches 200 at @p[gamemode=survival] positioned ^ ^ ^5 run summon wither ~ ~5 ~ {Tags:["yf_lord","yf_hades"], CustomName:'"Hades"', CustomNameVisible:true, NoAI:1, Silent:1}
...
execute if score @s YouFools matches 220 at @e[type=wither,tag=yf_lord] run playsound legacy.house.hades.intro master @a[distance=..100] ~ ~1 ~ 2 1 1
execute if score @s YouFools matches 220 run scoreboard players set $State YouFools 11
